...
describe EventDecorator do
  ...
  context "becoming familiar with decorators" do
    it "properly infers the decorator" do
      expect(event.decorate).to be_decorated_with EventDecorator
    end

    it "properly decorates by hand" do
      expect(EventDecorator.new(event)).to be_decorated_with EventDecorator
    end
  end

  context "#name" do
    it "display's the event's #longname attribute" do
      expect(subject.name).to eq event.longname
    end
  end
